...
//    there are three string in this element.three elements has same order in first chracter in a so,
//     a is removed both side, , then check it as second position in string, , , i think it is check by their assci value.c = 67 b = 66 a = 65

//   if as ascii value  is less then b and c so it is ordered by lexically  aass is the first order aback is the second order acomplash is the third order
...
{
  int i, j;
  char str[10][50] = {"golam Hakkani", "mim Hakkani", "rita", "Abbas Khan", "boyzed", "rina rao"};
  char temp[50];
